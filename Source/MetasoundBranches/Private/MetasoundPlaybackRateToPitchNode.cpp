...
#include "MetasoundBranches/Public/MetasoundPlaybackRateToPitchNode.h"
...
#define LOCTEXT_NAMESPACE "MetasoundBranches_PlaybackRateToPitchNode"

namespace Metasound
{
	namespace PlaybackRateToPitchVertexNames
	{
		METASOUND_PARAM(PlaybackRate, "Playback Rate", "Playback rate to convert to pitch.")
		...
		METASOUND_PARAM(Pitch, "Pitch", "Resulting MIDI note pitch (float, unquantized).")
	}

	namespace PlaybackRateToPitchPrivate
	{
		class FPlaybackRateToPitchOperatorData final : public TOperatorData<FPlaybackRateToPitchOperatorData>
		{
		public:
			static const FLazyName OperatorDataTypeName;

			explicit FPlaybackRateToPitchOperatorData(const EMetaSoundPlaybackRateToPitchTuningMode InTuningMode)
				: TuningMode(InTuningMode)
			{
			}

			EMetaSoundPlaybackRateToPitchTuningMode TuningMode;
		};

		const FLazyName FPlaybackRateToPitchOperatorData::OperatorDataTypeName = TEXT("FPlaybackRateToPitchOperatorData");

		void AddTuningCentsInputs(FInputVertexInterface& InInput)
		{
			using namespace PlaybackRateToPitchVertexNames;
		...
		FVertexInterface GetVertexInterface(const EMetaSoundPlaybackRateToPitchTuningMode InTuningMode)
		{
			using namespace PlaybackRateToPitchVertexNames;

			FInputVertexInterface InputInterface(
				TInputDataVertex<float>(METASOUND_GET_PARAM_NAME_AND_METADATA(PlaybackRate), 1.0f),
				TInputDataVertex<float>(METASOUND_GET_PARAM_NAME_AND_METADATA(OriginalPitch), 69.0f));

			switch (InTuningMode)
			{
			case EMetaSoundPlaybackRateToPitchTuningMode::EDO:
				InputInterface.Add(TInputDataVertex<int32>(METASOUND_GET_PARAM_NAME_AND_METADATA(Divisions), 12));
				break;
			case EMetaSoundPlaybackRateToPitchTuningMode::TuningCents:
			...
			FOutputVertexInterface OutputInterface(
				TOutputDataVertex<float>(METASOUND_GET_PARAM_NAME_AND_METADATA(Pitch)));
		...
		float GetPlaybackRateForPitch(
			const float InPitch,
			const float InOriginalPitch,
			const int32 InDivisions,
			const float InCents[12])
		{
			const float PitchAdjusted = InPitch + GetTuningCentsForPitch(InPitch, InCents) / 100.0f;
			const float OriginalAdjusted = InOriginalPitch + GetTuningCentsForPitch(InOriginalPitch, InCents) / 100.0f;
			return FMath::Pow(2.0f, (PitchAdjusted - OriginalAdjusted) / 12.0f);
		}
	}

	class FPlaybackRateToPitchOperator : public TExecutableOperator<FPlaybackRateToPitchOperator>
	{
	public:
		using FPlaybackRateToPitchOperatorData = PlaybackRateToPitchPrivate::FPlaybackRateToPitchOperatorData;

		FPlaybackRateToPitchOperator(
			const TSharedPtr<const FPlaybackRateToPitchOperatorData>& InOperatorData,
			...
			, Pitch(TDataWriteReference<float>::CreateNew(69.0f))
		{
		}

		static const FVertexInterface& DeclareVertexInterface()
		{
			static const FVertexInterface Interface = PlaybackRateToPitchPrivate::GetVertexInterface(EMetaSoundPlaybackRateToPitchTuningMode::EqualTemperament12);
			return Interface;
		}
		...
		static FNodeClassMetadata GetNodeInfo()
		{
			...
			Metadata.ClassName = { TEXT("UE"), TEXT("PlaybackRateToPitch"), TEXT("Float") };
			Metadata.MajorVersion = 1;
			Metadata.MinorVersion = 1;
			Metadata.DisplayName = LOCTEXT("DisplayName", "Playback Rate to Pitch");
			Metadata.Description = LOCTEXT("Description", "Converts playback rate to MIDI pitch with selectable 12-TET, EDO, or per-note tuning cents mapping.");
		...
		{
			using namespace PlaybackRateToPitchVertexNames;
			InOutVertexData.BindReadVertex(METASOUND_GET_PARAM_NAME(PlaybackRate), PlaybackRate);
		...
		{
			using namespace PlaybackRateToPitchVertexNames;
			InOutVertexData.BindWriteVertex(METASOUND_GET_PARAM_NAME(Pitch), Pitch);
		}
		...
		static TUniquePtr<IOperator> CreateOperator(const FBuildOperatorParams& InParams, FBuildResults& OutErrors)
		{
			...
			const FPlaybackRateToPitchOperatorData* ConfigData = CastOperatorData<const FPlaybackRateToPitchOperatorData>(InParams.Node.GetOperatorData().Get());
			if (!ConfigData)
			{
				return MakeUnique<FNoOpOperator>();
			}

			const TSharedPtr<const FPlaybackRateToPitchOperatorData>& OperatorDataSharedPtr =
				StaticCastSharedPtr<const FPlaybackRateToPitchOperatorData>(InParams.Node.GetOperatorData());

			auto InPlaybackRate = InParams.InputData.GetOrCreateDefaultDataReadReference<float>(METASOUND_GET_PARAM_NAME(PlaybackRate), InParams.OperatorSettings);
			...
			return MakeUnique<FPlaybackRateToPitchOperator>(
		...
		void Execute()
		{
			const float Rate = FMath::Max(KINDA_SMALL_NUMBER, *PlaybackRate);
			...
			case EMetaSoundPlaybackRateToPitchTuningMode::EDO:
			{
				...
				*Pitch = *OriginalPitch + static_cast<float>(SafeDivisions) * FMath::Log2(Rate);
				break;
			}
			case EMetaSoundPlaybackRateToPitchTuningMode::TuningCents:
			{
				...
				float Estimate = *OriginalPitch + 12.0f * FMath::Log2(Rate);
				for (int32 Iteration = 0; Iteration < 8; ++Iteration)
				{
					const float CurrentRate = PlaybackRateToPitchPrivate::GetPlaybackRateForPitch(Estimate, *OriginalPitch, 12, Cents);
					const float Error = CurrentRate - Rate;
					...
					const float RatePlus = PlaybackRateToPitchPrivate::GetPlaybackRateForPitch(Estimate + Epsilon, *OriginalPitch, 12, Cents);
					const float RateMinus = PlaybackRateToPitchPrivate::GetPlaybackRateForPitch(Estimate - Epsilon, *OriginalPitch, 12, Cents);
					const float Derivative = (RatePlus - RateMinus) / (2.0f * Epsilon);
			...
			default:
				*Pitch = *OriginalPitch + 12.0f * FMath::Log2(Rate);
				break;
			}
		}

	private:
		TSharedPtr<const FPlaybackRateToPitchOperatorData> OperatorData;
		FFloatReadRef PlaybackRate;
		...
		TDataWriteReference<float> Pitch;
	};

	using FPlaybackRateToPitchNode = TNodeFacade<FPlaybackRateToPitchOperator>;
	METASOUND_REGISTER_NODE_AND_CONFIGURATION(FPlaybackRateToPitchNode, FMetaSoundPlaybackRateToPitchNodeConfiguration);
}

FMetaSoundPlaybackRateToPitchNodeConfiguration::FMetaSoundPlaybackRateToPitchNodeConfiguration()
	: OperatorData(MakeShared<Metasound::PlaybackRateToPitchPrivate::FPlaybackRateToPitchOperatorData>(TuningMode))
{
}

TInstancedStruct<FMetasoundFrontendClassInterface> FMetaSoundPlaybackRateToPitchNodeConfiguration::OverrideDefaultInterface(const FMetasoundFrontendClass& InClass) const
{
	return TInstancedStruct<FMetasoundFrontendClassInterface>::Make(
		FMetasoundFrontendClassInterface::GenerateClassInterface(
			Metasound::PlaybackRateToPitchPrivate::GetVertexInterface(TuningMode)));
}

TSharedPtr<const Metasound::IOperatorData> FMetaSoundPlaybackRateToPitchNodeConfiguration::GetOperatorData() const
{
